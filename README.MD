...
2.  **Modo Servidor (Standalone):**
    * O servidor é iniciado manualmente no terminal (`node server.js`).
    * **O primeiro utilizador** a aceder ao endereço do servidor através de um browser torna-se automaticamente o anfitrião.
    * Todos os utilizadores subsequentes que acederem ao mesmo endereço serão participantes.
...
3.  **Iniciar a aplicação (escolha um dos modos):**
    * **Modo Anfitrião (Recomendado):**
        ```bash
        npm start
        ```
        Este comando iniciará o servidor e a janela da aplicação Electron para o anfitrião.
    * **Modo Servidor:**
        ```bash
        node server.js
        ```
        Este comando iniciará apenas o servidor. O primeiro utilizador a aceder ao endereço IP do servidor no browser será o anfitrião.